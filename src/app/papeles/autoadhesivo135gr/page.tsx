...
import { Sparkles, CheckCircle2, MessageCircle, ArrowRight, ShieldCheck, Tag, StickyNote } from "lucide-react";
import preciosPorPapel from "@/utilidades/preciosPorPapel";

export default function PaginaAutoadhesivoComun() {
  const numeroWhatsapp = "5491123909529";
  const mensajeWsp = encodeURIComponent("Hola! Quisiera consultar para imprimir en Autoadhesivo Común 135 Grs.");

  // Obtenemos únicamente el Autoadhesivo común (135 Grs)
  const papelAdhesivo = preciosPorPapel.find((p) => p.id === "adhesivoComun");
  const precioPapel = papelAdhesivo?.precio || 1495;
      ...
      <div className="flex-1 w-full max-w-2xl bg-white dark:bg-slate-800 p-8 rounded-xl shadow-lg border border-violet-100 dark:border-slate-700 transition-colors duration-300 mx-auto">
        ...
        <div className="flex justify-center mb-3">
          <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-violet-100 dark:bg-violet-950 text-violet-700 dark:text-violet-300 text-xs font-bold uppercase tracking-wide border border-violet-200 dark:border-violet-800">
            <StickyNote className="w-3.5 h-3.5" /> Gramaje 135 Grs - Uso General
          </span>
        </div>

        {/* TÍTULO PRINCIPAL */}
        <h2 className="text-2xl sm:text-3xl font-bold text-violet-700 dark:text-violet-400 mb-3 text-center">
          Autoadhesivo Común (135 Grs)
        </h2>
        
        <p className="text-slate-600 dark:text-slate-300 text-center text-sm mb-6">
          La opción económica e ideal para etiquetas de frascos, stickers promocionales de interior, packaging, rotulado de productos y souvenirs.
        </p>
        ...
        <div className="mb-8 perspective-[1000px] flex flex-col items-center">
          ...
          >
            {/* Imagen A4 Vertical (/public/autoadhesivo-comun.webp) */}
            <Image
              src="/global-autadhesivo-135grA4.png" 
              alt="Muestra de Autoadhesivo Común 135 Grs"
              fill
              priority
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            <div className="p-4 rounded-xl bg-slate-50 dark:bg-slate-700/50 border border-violet-100 dark:border-slate-600 space-y-1">
              <h3 className="font-bold text-slate-800 dark:text-slate-200 text-sm flex items-center gap-1.5">
                <CheckCircle2 className="w-4 h-4 text-emerald-500" /> Gran Adherencia
              </h3>
              <p className="text-xs text-slate-500 dark:text-slate-400">
                Adhesivo de excelente fijación sobre papel, cartón, plástico y superficies limpias.
              </p>
            </div>

            <div className="p-4 rounded-xl bg-slate-50 dark:bg-slate-700/50 border border-violet-100 dark:border-slate-600 space-y-1">
              <h3 className="font-bold text-slate-800 dark:text-slate-200 text-sm flex items-center gap-1.5">
                <ShieldCheck className="w-4 h-4 text-violet-600 dark:text-violet-400" /> Ideal Uso Comercial
              </h3>
              <p className="text-xs text-slate-500 dark:text-slate-400">
                Terminación prolija y versátil con excelente relación precio-calidad.
              </p>
            </div>
          </div>
          ...
          <div className="p-4 rounded-xl bg-violet-50/50 dark:bg-slate-700/30 border border-violet-200 dark:border-slate-600 flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <span className="text-xs text-violet-600 dark:text-violet-400 font-semibold">Autoadhesivo Común (135 Grs)</span>
              </div>
            </div>
