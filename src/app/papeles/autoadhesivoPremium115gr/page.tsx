...
import { Sparkles, CheckCircle2, MessageCircle, ArrowRight, ShieldCheck, Tag, Star } from "lucide-react";
import preciosPorPapel from "@/utilidades/preciosPorPapel";

export default function PaginaAutoadhesivoPremium() {
  const numeroWhatsapp = "5491123909529";
  const mensajeWsp = encodeURIComponent("Hola! Quisiera consultar para imprimir en Autoadhesivo Premium 115 Grs.");

  // Obtenemos únicamente el Autoadhesivo Premium
  const papelAdhesivo = preciosPorPapel.find((p) => p.id === "adhesivoPremium");
  const precioPapel = papelAdhesivo?.precio || 1695;
      ...
      <div className="flex-1 w-full max-w-2xl bg-white dark:bg-slate-800 p-8 rounded-xl shadow-lg border border-violet-100 dark:border-slate-700 transition-colors duration-300 mx-auto">
        ...
        <div className="flex justify-center mb-3">
          <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-amber-100 dark:bg-amber-950 text-amber-700 dark:text-amber-300 text-xs font-bold uppercase tracking-wide border border-amber-200 dark:border-amber-800">
            <Star className="w-3.5 h-3.5 fill-current" /> Acabado Ilustración Premium - 115 Grs
          </span>
        </div>

        {/* TÍTULO PRINCIPAL */}
        <h2 className="text-2xl sm:text-3xl font-bold text-violet-700 dark:text-violet-400 mb-3 text-center">
          Autoadhesivo Premium (115 Grs)
        </h2>
        
        <p className="text-slate-600 dark:text-slate-300 text-center text-sm mb-6">
          Papel ilustración autoadhesivo de excelente definición cromática y brillo satinado. La mejor opción para etiquetas de productos gourmet, marcas de autor, souvenirs finos y packaging de alto nivel.
        </p>
        ...
        <div className="mb-8 perspective-[1000px] flex flex-col items-center">
          ...
          >
            {/* Imagen A4 Vertical (/public/autoadhesivo-premium.webp) */}
            <Image
              src="/arjet-autoadhesivoPremium-115grA4.png" 
              alt="Muestra de Autoadhesivo Premium 115 Grs"
              fill
              priority
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            <div className="p-4 rounded-xl bg-slate-50 dark:bg-slate-700/50 border border-violet-100 dark:border-slate-600 space-y-1">
              <h3 className="font-bold text-slate-800 dark:text-slate-200 text-sm flex items-center gap-1.5">
                <Sparkles className="w-4 h-4 text-amber-500" /> Colores Vivos y Brillantes
              </h3>
              <p className="text-xs text-slate-500 dark:text-slate-400">
                Superficie satinada con recubrimiento especial para lograr la mayor intensidad cromática.
              </p>
            </div>

            <div className="p-4 rounded-xl bg-slate-50 dark:bg-slate-700/50 border border-violet-100 dark:border-slate-600 space-y-1">
              <h3 className="font-bold text-slate-800 dark:text-slate-200 text-sm flex items-center gap-1.5">
                <ShieldCheck className="w-4 h-4 text-violet-600 dark:text-violet-400" /> Adhesivo Reforzado
              </h3>
              <p className="text-xs text-slate-500 dark:text-slate-400">
                Lámina delgada de 115 Grs que se amolda perfecto a frascos, botellas y cajas de cartón.
              </p>
            </div>
          </div>
          ...
          <div className="p-4 rounded-xl bg-violet-50/50 dark:bg-slate-700/30 border border-violet-200 dark:border-slate-600 flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <span className="text-xs text-violet-600 dark:text-violet-400 font-semibold">Autoadhesivo Premium (115 Grs)</span>
              </div>
            </div>
