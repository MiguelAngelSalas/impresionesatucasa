...
export default function PaginaOpalina210() {
  const numeroWhatsapp = "5491123909529";
  const mensajeWsp = encodeURIComponent("Hola! Quisiera consultar para imprimir en Mate / Opalina Gruesa 210 Grs.");

  // Obtenemos únicamente el papel Mate grueso / Opalina (210 Grs)
  const papelMateGrueso = preciosPorPapel.find((p) => p.id === "mateGrueso");
  const precioPapel = papelMateGrueso?.precio || 1455;
      ...
      <div className="flex-1 w-full max-w-2xl bg-white dark:bg-slate-800 p-8 rounded-xl shadow-lg border border-violet-100 dark:border-slate-700 transition-colors duration-300 mx-auto">
        ...
        <div className="flex justify-center mb-3">
          <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-violet-100 dark:bg-violet-950 text-violet-700 dark:text-violet-300 text-xs font-bold uppercase tracking-wide border border-violet-200 dark:border-violet-800">
            <FileText className="w-3.5 h-3.5" /> Gramaje 210 Grs - Rígido Mate
          </span>
        </div>

        {/* TÍTULO PRINCIPAL */}
        <h2 className="text-2xl sm:text-3xl font-bold text-violet-700 dark:text-violet-400 mb-3 text-center">
          Mate / Opalina Gruesa (210 Grs)
        </h2>
        
        <p className="text-slate-600 dark:text-slate-300 text-center text-sm mb-6">
          Papel de alto cuerpo y rigidez con acabado mate suave. La mejor alternativa para láminas decorativas, portadas de informes, certificados, tarjetas simples e ilustraciones sin reflejos.
        </p>
        ...
        <div className="mb-8 perspective-[1000px] flex flex-col items-center">
          ...
          >
            {/* Imagen A4 Vertical (/public/mate-210.webp) */}
            <Image
              src="/global-mateGruesoSimple-210grA4.png" 
              alt="Muestra de Papel Mate Grueso / Opalina 210 Grs"
              fill
              priority
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            <div className="p-4 rounded-xl bg-slate-50 dark:bg-slate-700/50 border border-violet-100 dark:border-slate-600 space-y-1">
              <h3 className="font-bold text-slate-800 dark:text-slate-200 text-sm flex items-center gap-1.5">
                <CheckCircle2 className="w-4 h-4 text-emerald-500" /> Gran Firmeza y Cuerpo
              </h3>
              <p className="text-xs text-slate-500 dark:text-slate-400">
                Soporte rígido de 210 Grs que mantiene la forma sin doblarse.
              </p>
            </div>

            <div className="p-4 rounded-xl bg-slate-50 dark:bg-slate-700/50 border border-violet-100 dark:border-slate-600 space-y-1">
              <h3 className="font-bold text-slate-800 dark:text-slate-200 text-sm flex items-center gap-1.5">
                <ShieldCheck className="w-4 h-4 text-violet-600 dark:text-violet-400" /> Terminación Antirreflejo
              </h3>
              <p className="text-xs text-slate-500 dark:text-slate-400">
                Superficie lisa y opaca que permite apreciar el diseño sin encandilamientos.
              </p>
            </div>
          </div>
          ...
          <div className="p-4 rounded-xl bg-violet-50/50 dark:bg-slate-700/30 border border-violet-200 dark:border-slate-600 flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <span className="text-xs text-violet-600 dark:text-violet-400 font-semibold">Mate Grueso / Opalina (210 Grs)</span>
              </div>
            </div>
