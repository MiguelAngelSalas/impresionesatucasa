...
import { Sparkles, CheckCircle2, MessageCircle, ArrowRight, ShieldCheck, Tag, Droplets } from "lucide-react";
import preciosPorPapel from "@/utilidades/preciosPorPapel";

export default function PaginaViniloResistente() {
  const numeroWhatsapp = "5491123909529";
  const mensajeWsp = encodeURIComponent("Hola! Quisiera consultar para imprimir Stickers en Vinilo Resistente al Agua.");

  // Obtenemos únicamente el Vinilo Resistente al agua (Stickers)
  const papelStickers = preciosPorPapel.find((p) => p.id === "styckers");
  const precioPapel = papelStickers?.precio || 2130;
    ...
    <div className="flex flex-col md:flex-row gap-8 items-start justify-center max-w-6xl mx-auto px-4 py-10">
      ...
      <div className="flex-1 w-full max-w-2xl bg-white dark:bg-slate-800 p-8 rounded-xl shadow-lg border border-violet-100 dark:border-slate-700 transition-colors duration-300 mx-auto">
        ...
        <div className="flex justify-center mb-3">
          <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-cyan-100 dark:bg-cyan-950 text-cyan-700 dark:text-cyan-300 text-xs font-bold uppercase tracking-wide border border-cyan-200 dark:border-cyan-800">
            <Droplets className="w-3.5 h-3.5" /> 100% Water Resistant
          </span>
        </div>

        {/* TÍTULO PRINCIPAL */}
        <h2 className="text-2xl sm:text-3xl font-bold text-violet-700 dark:text-violet-400 mb-3 text-center">
          Autoadhesivo Resistente al Agua (Stickers)
        </h2>
        
        <p className="text-slate-600 dark:text-slate-300 text-center text-sm mb-6">
          Vinilo adhesivo de alta resistencia para termo, termos de café, laptops, cascos y packaging. Soporta salpicaduras, lavados y roces cotidianos sin perder el color.
        </p>
        ...
        <div className="mb-8 perspective-[1000px] flex flex-col items-center">
          ...
          >
            {/* Imagen A4 Vertical (/public/vinilo-resistente.jpg) */}
            <Image
              src="/arjet-filmiloBlancoResistente-88micronesA4.png" 
              alt="Muestra de Autoadhesivo Resistente al Agua"
              fill
              priority
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            <div className="p-4 rounded-xl bg-slate-50 dark:bg-slate-700/50 border border-violet-100 dark:border-slate-600 space-y-1">
              <h3 className="font-bold text-slate-800 dark:text-slate-200 text-sm flex items-center gap-1.5">
                <Droplets className="w-4 h-4 text-cyan-500" /> Resistente a Líquidos
              </h3>
              <p className="text-xs text-slate-500 dark:text-slate-400">
                La tinta no se corre ni se degrada en contacto con agua o humedad.
              </p>
            </div>

            <div className="p-4 rounded-xl bg-slate-50 dark:bg-slate-700/50 border border-violet-100 dark:border-slate-600 space-y-1">
              <h3 className="font-bold text-slate-800 dark:text-slate-200 text-sm flex items-center gap-1.5">
                <ShieldCheck className="w-4 h-4 text-violet-600 dark:text-violet-400" /> Gran Adherencia
              </h3>
              <p className="text-xs text-slate-500 dark:text-slate-400">
                Fijación firme sobre plástico, vidrio, metal y superficies curvas.
              </p>
            </div>
          </div>
          ...
          <div className="p-4 rounded-xl bg-violet-50/50 dark:bg-slate-700/30 border border-violet-200 dark:border-slate-600 flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <span className="text-xs text-violet-600 dark:text-violet-400 font-semibold">Resistente al Agua (Stickers)</span>
              </div>
            </div>
