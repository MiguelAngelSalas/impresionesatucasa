...
import { Image as ImageIcon, CheckCircle2, MessageCircle, ArrowRight, ShieldCheck, Sparkles, Tag } from "lucide-react";
import preciosPorPapel from "@/utilidades/preciosPorPapel";

export default function PaginaPapelFotoGrueso() {
  const numeroWhatsapp = "5491123909529";
  const mensajeWsp = encodeURIComponent("Hola! Quisiera consultar para imprimir en Papel Fotográfico Grueso 200 Grs.");

  // Obtenemos únicamente el papel Foto Grueso (200 Grs)
  const papelFotoGrueso = preciosPorPapel.find((p) => p.id === "fotoGrueso");
  const precioPapel = papelFotoGrueso?.precio || 1540;
    ...
    <div className="flex flex-col md:flex-row gap-8 items-start justify-center max-w-6xl mx-auto px-4 py-10">
      ...
      <div className="flex-1 w-full max-w-2xl bg-white dark:bg-slate-800 p-8 rounded-xl shadow-lg border border-violet-100 dark:border-slate-700 transition-colors duration-300 mx-auto">
        ...
        <h2 className="text-2xl sm:text-3xl font-bold text-violet-700 dark:text-violet-400 mb-3 text-center">
          Papel Fotográfico Grueso (200 Grs)
        </h2>
        
        <p className="text-slate-600 dark:text-slate-300 text-center text-sm mb-6">
          Soporte rígido de gran cuerpo, colores intensos y terminación brillante. La opción estándar elegida para cuadros, fotos de regalo, láminas e ilustraciones.
        </p>
        ...
        <div className="mb-8 perspective-[1000px] flex flex-col items-center">
          ...
          >
            <Image
              src="/arjet-fotoGrueso-200grA4.png" 
              alt="Muestra de Papel Fotográfico Grueso 200 Grs"
              fill
              priority
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            <div className="p-4 rounded-xl bg-slate-50 dark:bg-slate-700/50 border border-violet-100 dark:border-slate-600 space-y-1">
              <h3 className="font-bold text-slate-800 dark:text-slate-200 text-sm flex items-center gap-1.5">
                <CheckCircle2 className="w-4 h-4 text-emerald-500" /> Fidelidad de Color
              </h3>
              <p className="text-xs text-slate-500 dark:text-slate-400">
                Tintas calibradas para una óptima nitidez y riqueza de tono.
              </p>
            </div>

            <div className="p-4 rounded-xl bg-slate-50 dark:bg-slate-700/50 border border-violet-100 dark:border-slate-600 space-y-1">
              <h3 className="font-bold text-slate-800 dark:text-slate-200 text-sm flex items-center gap-1.5">
                <ShieldCheck className="w-4 h-4 text-violet-600 dark:text-violet-400" /> Cuerpo y Firmeza
              </h3>
              <p className="text-xs text-slate-500 dark:text-slate-400">
                Gramaje de 200 Grs rígido, sin pliegues al manipular.
              </p>
            </div>
          </div>
          ...
          <div className="p-4 rounded-xl bg-violet-50/50 dark:bg-slate-700/30 border border-violet-200 dark:border-slate-600 flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <span className="text-xs text-violet-600 dark:text-violet-400 font-semibold">Foto Grueso 200 Grs</span>
              </div>
            </div>
